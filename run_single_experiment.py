...
from dataset import Dataset, TimestampsSampler
...
@torch.no_grad()
def evaluate_on_val_or_test_dataloaders_draft(model, dataloader, split, timestamps_loader, loss_fn, metric, amp=True):
    preds = []
    for timestamps_batch in timestamps_loader:
        padded = False
        if len(timestamps_batch) != dataset.eval_batch_size:
...
@torch.no_grad()
def evaluate_with_dataloader_logic_draft(model, dataset, val_loader, test_loader, loss_fn, metric, amp=True,
             do_not_evaluate_on_test=False):
    metrics = {}
    val_metric = evaluate_on_val_or_test_dataloaders_draft(model=model, dataset=dataset, split='val',
                                         dataloa=val_timestamps_loader, loss_fn=loss_fn,
                                         metric=metric, amp=amp)
    metrics[f'val {metric}'] = val_metric

    if not do_not_evaluate_on_test:
        test_metric = evaluate_on_val_or_test_dataloaders_draft(model=model, dataset=dataset, split='test',
                                              timestamps_loader=test_timestamps_loader, loss_fn=loss_fn,
                                              metric=metric, amp=amp)
        metrics[f'test {metric}'] = test_metric
...
def train_with_timestamps_sampler_draft(model, dataset, loss_fn, metric, logger: Logger, num_epochs, num_accumulation_steps, eval_every, lr,
    ...
    num_steps = len(train_sampler) * num_epochs
        ...
        for epoch in range(state_handler.epochs_finished+1, num_epochs):
            for cur_train_timestamps_batch in train_loader:
                ...
                if optimizer_steps_till_eval == 0:
                    ...
                    model.eval()
                    metrics = evaluate_with_dataloader_logic_draft(
                        model=model, dataset=dataset, val_loader=val_loader,
                        test_loader=test_loader, loss_fn=loss_fn, metric=metric,
            ...
            # reinitialize train_loader as it could be used with only the fraction of data:
