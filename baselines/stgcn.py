...
class STGCN(BaseModel):
    ...
    '''
    def __init__(self, laplacian_matrix, blocks, temporal_kernel_size, spatial_kernel_size, dropout, **args):
        super(STGCN, self).__init__(**args)
        modules = []
        for l in range(len(blocks) - 3):
            modules.append(
                STConvBlock(temporal_kernel_size, spatial_kernel_size,
                            self.node_num, blocks[l][-1], blocks[l + 1],
                            laplacian_matrix, dropout)
            )
...
class STConvBlock(nn.Module):
    def __init__(self, Kt, Ks, node_num, last_block_channel, channels, laplacian_matrix, dropout):
        super(STConvBlock, self).__init__()
        self.tmp_conv1 = TemporalConvLayer(Kt, last_block_channel, channels[0], node_num)
        self.graph_conv = GraphConvLayer(channels[0], channels[1], Ks, laplacian_matrix)
...
class GraphConvLayer(nn.Module):
    def __init__(self, c_in, c_out, Ks, laplacian_matrix):
        ...
        self.cheb_graph_conv = ChebGraphConv(c_out, c_out, Ks, laplacian_matrix)
...
class ChebGraphConv(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.Ks - 1 < 0:
            ...
            x_0 = x
            x_1 = torch.einsum('hi,btij->bthj', self.laplacian_matrix, x)
            x_list = [x_0, x_1]
        elif self.Ks - 1 >= 2:
            x_0 = x
            x_1 = torch.einsum('hi,btij->bthj', self.laplacian_matrix, x)
            ...
            for k in range(2, self.Ks):
                x_list.append(torch.einsum('hi,btij->bthj', 2 * self.laplacian_matrix, x_list[k - 1]) - x_list[k - 2])
