...
class STGODE(BaseModel):
    ...
    '''
    def __init__(self, hidden_dim, transition_matrix, **args):     
        super(STGODE, self).__init__(**args)
        # spatial graph
        self.sp_blocks = nn.ModuleList(
            [nn.Sequential(
                STGCNBlock(in_channels=self.input_dim, out_channels=[hidden_dim, hidden_dim, hidden_dim],
                node_num=self.node_num, seq_len=self.seq_len, A_hat=transition_matrix),
                STGCNBlock(in_channels=hidden_dim, out_channels=[hidden_dim, hidden_dim, hidden_dim],
                node_num=self.node_num, seq_len=self.seq_len, A_hat=transition_matrix)) for _ in range(3)
            ])

        # NOTE: these objects are used in the removed code
        # # semantic graph
    ...
    def forward(self, x, label=None):
        ...
        # NOTE: we do not have semantic adjacency matrix
        # # semantic graph
...
class STGCNBlock(nn.Module):
    def __init__(self, in_channels, out_channels, node_num, seq_len, A_hat):
