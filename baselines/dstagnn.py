...
class DSTAGNN(BaseModel):
    ...
    NUM_CHEB_FILTERS = 32
    NUM_TEMPORAL_FILTERS = 32
    TEMPORAL_STRIDE = 1
    KEY_VALUE_DIM = 32
    NUM_HEADS = 3

    def __init__(self, cheb_polynomials, adjacency_matrix, hidden_dim, num_blocks, **args):
        super(DSTAGNN, self).__init__(**args)

        # NOTE: no idea why camel case is used here
        self.BlockList = nn.ModuleList(
            [
                DSTAGNN_block(self.input_dim, self.input_dim,
                              len(cheb_polynomials), self.NUM_CHEB_FILTERS,
                              self.NUM_TEMPORAL_FILTERS, self.TEMPORAL_STRIDE,
                              cheb_polynomials, adjacency_matrix, self.node_num, self.seq_len,
                              hidden_dim, self.KEY_VALUE_DIM, self.KEY_VALUE_DIM, self.NUM_HEADS)
            ] + [
                DSTAGNN_block(self.input_dim * self.NUM_TEMPORAL_FILTERS, self.NUM_CHEB_FILTERS,
                              len(cheb_polynomials), self.NUM_CHEB_FILTERS,
                              self.NUM_TEMPORAL_FILTERS, 1,
                              cheb_polynomials, adjacency_matrix, self.node_num, self.seq_len // self.TEMPORAL_STRIDE,
                              hidden_dim, self.KEY_VALUE_DIM, self.KEY_VALUE_DIM, self.NUM_HEADS)
...
class DSTAGNN_block(nn.Module):
    def __init__(self, num_of_d, in_channels, K, nb_chev_filter, nb_time_filter, time_stride,
