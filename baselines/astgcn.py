...
class ASTGCN(BaseModel):
    ...
    def __init__(self, cheb_polynomials, hidden_dim, num_blocks, **args):
        super(ASTGCN, self).__init__(**args)

        self.BlockList = nn.ModuleList([
            ASTGCN_block(self.input_dim, len(cheb_polynomials), hidden_dim, hidden_dim, self.TEMPORAL_STRIDE,
                         cheb_polynomials, self.node_num, self.seq_len)
        ])
        self.BlockList.extend([
            ASTGCN_block(hidden_dim, len(cheb_polynomials), hidden_dim, hidden_dim, 1,
                         cheb_polynomials, self.node_num, self.seq_len // self.TEMPORAL_STRIDE)
...
class ASTGCN_block(nn.Module):
    def __init__(self, in_channels, order, nb_chev_filter, nb_time_filter, time_strides, cheb_polynomials, node_num, seq_len):
