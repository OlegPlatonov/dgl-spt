...
def calculate_features_statistics_slice(memmap_path, memmap_shape, slice_start_idx, slice_end_idx, first_val_index):
    ...
    memmap_slice = np.memmap(memmap_path, mode="r", dtype="float32", shape=memmap_shape)

    empty_features_mask = np.isclose(memmap_slice[slice_start_idx:slice_end_idx, ...], 0).sum(-1) == memmap_shape[-1]


    if slice_start_idx >= first_val_index:
        nonempty_features_amounts_train = 0
        first_momentum_features, second_momentum_features, maxes_features, mins_features = np.zeros(memmap_shape[-1]), np.zeros(memmap_shape[-1]), np.zeros(memmap_shape[-1]), np.zeros(memmap_shape[-1])
    else:
        slice_end_idx = min(slice_end_idx, first_val_index)
        train_features_slice = memmap_slice[slice_start_idx:slice_end_idx, ...]

        first_momentum_features = train_features_slice.sum((0, 1))
        second_momentum_features = (train_features_slice ** 2).sum((0, 1))
    ...
    print(f"Calculated statistics for slice [{slice_start_idx}, {slice_end_idx})")
    return (
...
def main():
    ...
    # original_memmap_features = np.memmap(initial_spatiotemporal_features_path, mode="r", dtype="float32", shape=memmap_shape)
    ...
    for args in _scaling_arguments:
        process_features_based_on_statistics(*args)
    # with mp.Pool(processes=args.processes) as pool:
